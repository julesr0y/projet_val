...
void updateRameText(Text& text, vector<Rame>& tabRame, Rame rame, int pos_x) {
    ...
    for (size_t i = 0; i < tabRame.size(); i++) {
        if (rame.get_numero() == tabRame[i].get_numero()) {
            ...
            tableauTexte += "Rame N° : " + to_string(tabRame[i].get_numero()) + " | " + "Passagers : " + to_string(rame.get_passagers()) + "\n";
        }
        else {
            tableauTexte += "Rame N° : " + to_string(tabRame[i].get_numero()) + " | " + "Passagers : " + to_string(tabRame[i].get_passagers()) + "\n";
        }
    }
