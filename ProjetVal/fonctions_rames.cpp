...
void moveRame(Rame& rame, Rame& rame_apres, vector<Station> listeStations, bool beginning, vector<Rame>& tabRame, Text& text) {
    ...
    bool reverseOrder = false; //par d�faut, on inverse pas la liste des stations de la ligne

    while (true) { //boucle infinie, pour recommencer ind�finiment le parcours de la ligne pour la rame
        rame.setFreinage(false); //par d�faut, on set la rame comme sur la voie (pas en direction de station blanche)
        if (reverseOrder) { //si l'inversement de liste des stations de la ligne est activ�
        ...
        for (int i = 0; i < listeStations.size(); i++) {
            ...
            if (i == listeStations.size() - 1) { //si on est � la derni�re station
                rame.setFreinage(true); //si la rame se dirige vers une station blanche, on la set en mode freinage
            }
            else if (i == listeStations.size() - 2) { //si on est � la derni�re station avant le terminus
            ...
            float end_pos_x = listeStations[i].getPositionX(); //position x � atteindre (station suivante)
            float end_pos_y = listeStations[i].getPositionY() - 3; //position y � atteindre (station suivante)
            ...
            //position d'arret a la station en fonction de direction pr�c�dente et suivante
            if (i != 0 && i < listeStations.size()) {
                if (!rame.getRetour()) {
                    ...
                    //descente apr�s int�rieur droite
            ...
            while ((rame.get_position_x() != end_pos_x) || (rame.get_position_y() != end_pos_y)) {
                ...
                bool authorize_move = true; //on autorise le d�placement
                ...
                if (rame.getHorizontal()) {
                    if (dist_entre_rames_x == 0 && beginning == false) { //�vite que des rames partent dans le mauvais ordre lors de la g�n�ration des threads
                        authorize_move = false;
                    }
                    if (distanceToStation_x >= (2.0 / 3.0) * dist_entre_2_stations_x) {
                        //on acc�l�re la rame sur le premier tiers de la distance
                        v = VITESSE_ACCELERATION;
                    }
                    else if (distanceToStation_x >= (1.0 / 3.0) * dist_entre_2_stations_x) {
                        //on maintient une vitesse constante pour la rame sur le deuxi�me tiers de la distance
                        v = VITESSE_CONSTANTE;
                    }
                    else if (distanceToStation_x < (1.0 / 3.0) * dist_entre_2_stations_x) {
                        //si une rame se rapproche d'une station (dernier tiers de la distance), on freine son d�placement
                        v = VITESSE_FREINAGE;
                    }
                    if (rame.isFreinage()) {
                    ...
                    if (dist_entre_rames_x < 150 && rame_apres.hasStarted() == true && rame_apres.getRetour() == rame.getRetour() && rame_apres.getHorizontal() == rame.getHorizontal() && rame_apres.get_position_y() == rame.get_position_y()) {
                        //si 2 rames se suivant sur une m�me voie ont une distance faible, on freine le deplacement de la rame derri�re
                        v = VITESSE_FREINAGE;
                    }
                    if (dist_entre_rames_x < 100 && rame_apres.hasStarted() == true && rame_apres.getRetour() == rame.getRetour() && rame_apres.getHorizontal() == rame.getHorizontal() && rame_apres.get_position_y() == rame.get_position_y()) {
                        //si 2 rames se suivant sur une m�me voie ont une distance trop faible, on interdit le deplacement de la rame derri�re
                        authorize_move = false;
                    }
                ...
                if(!rame.getHorizontal()) {
                    if (dist_entre_rames_y == 0 && beginning == false) { //�vite que des rames partent dans le mauvais ordre lors de la g�n�ration des threads
                        authorize_move = false;
                    }
                ...
                if (authorize_move) {
                    rame.setStarted(true); // on set la rame comme d�marr�e
                ...
                this_thread::sleep_for(chrono::milliseconds(v)); //r�glage de la vitesse de la rame
            }
            ...
            if (rame.estArrete() && i == listeStations.size() - 1 && !rame.getRetour()) {
                ...
                rame.setRetour(true); //on active le mode retour (voie sup�rieure)
            ...
            ////descend
                ...
                rame.rotate180(); //on tourne la rame de 180 degres
                rame.setRetour(false); //on enleve le mode retour (voie inf�rieure)
                reverseOrder = true; //on active l'inversion de la liste des stations
            }
        }
    }
